...
static PyMethodDef _playaudio_methods[] = {
...
static char doc_string [] = "_playaudio is the module containing the C-extension that handles the low-level OS-specific API interactions for audio playback.";

static struct PyModuleDef _playaudio_module = {
   PyModuleDef_HEAD_INIT,
   "_playaudio",   /* name of module */
...
PyMODINIT_FUNC
PyInit__playaudio(void)
{
    PyObject *m;

    m = PyModule_Create(&_playaudio_module);
    if (m == NULL)
        return NULL;

    sa_python_error = PyErr_NewException("_playaudio.playaudioError", NULL, NULL);
    Py_INCREF(sa_python_error);
    PyModule_AddObject(m, "playaudioError", sa_python_error);
